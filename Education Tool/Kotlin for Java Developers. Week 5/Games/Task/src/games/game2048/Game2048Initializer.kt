...
object RandomGame2048Initializer: Game2048Initializer<Int> {
    ...
    override fun nextValue(board: GameBoard<Int?>): Pair<Cell, Int>? {
        val emptyCells = board.getAllCells().filter { cell -> board[cell]?.equals(0) ?: true }
        return if (emptyCells.isNotEmpty())
            Pair(emptyCells[Random.nextInt(emptyCells.size)], RandomGame2048Initializer.generateRandomStartValue())
        else
            null
    }
}